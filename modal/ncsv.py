...
class NCSV(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        if 'bert' in self.bert_path:
            title_features = self.bert(title_inputid, attention_mask=title_mask)['last_hidden_state']#（batch,length,dim=768)
        ...
        if 'AltClip' in self.bert_path:
            _,title_features = self.bert.get_text_features(title_inputid, attention_mask=title_mask)#（batch,length,dim=1024)
            _,transcript_features = self.bert.get_text_features(transcript_mask, attention_mask=transcript_mask)
        text_features=title_features*self.title_trans_fusion_param+transcript_features*(1-self.title_trans_fusion_param)
        ...
        for i in range(social_text_inputid.shape[0]):
            if 'bert' in self.bert_path:
                fea = self.bert(social_text_inputid[i], attention_mask=social_text_mask[i])['pooler_output']  # (batch,sequence,768)
            if 'AltClip' in self.bert_path:
        ...
        video_features,text_features=self.co_attention_tv(v=video_features, s=text_features,
                                                             v_len=video_features.shape[1], s_len=text_features.shape[1],
