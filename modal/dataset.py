...
class NCSVDataset(Dataset):
    def __init__(self, data_path='../data', json_path='full_data.json', bert_path='../bert-base-chinese',
                 max_length=512, video_theme=None):
        self.data = pd.read_json(os.path.join(data_path, json_path), orient='records', lines=True)
        if video_theme is not None:
            self.data = self.data[self.data['theme'] == video_theme]
        if 'bert' in bert_path:
            self.tokenizer = BertTokenizer.from_pretrained(bert_path)
        if 'Clip' in bert_path:
            process = getattr(AltCLIP, "AltCLIPProcess").from_pretrained(
                os.path.join(bert_path, 'checkpoints/altclip-xlmr-l'))
